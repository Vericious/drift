...
class TestHtmlDeadCodeRemoval:
    """Tests for dead code removal in HTML reporter."""

    def test_html_reporter_output_unchanged(self, populated_report):
        """HTML output is identical after removing duplicate return."""
        reporter = DriftReporter(populated_report)
        output = reporter.report_html()
        # Verify key HTML structure is present
        assert "<!DOCTYPE html>" in output
        assert "<html" in output
        assert "Drift Report" in output
        assert "Generated by Drift v" in output
        assert "</html>" in output

    def test_no_unreachable_code_after_html_return(self):
        """report_html has no duplicate return html statements."""
        import inspect

        source = inspect.getsource(DriftReporter.report_html)
        # Check for duplicate "return html" at the end of the function
        # After the dedent that closes the method, there should be no more "return html"
        lines = source.split("\n")
        return_html_lines = [
            (i, line) for i, line in enumerate(lines) if "return html" in line
        ]
        # There should be exactly 1 "return html" at the top level of the function
        # (nested functions may have their own returns)
        assert len(return_html_lines) == 1, (
            f"Expected 1 'return html' at top level, found {len(return_html_lines)}: "
            f"{return_html_lines}"
        )
