...
class TestJobsFlag:
    """Tests for -j/--jobs CLI flag for parallel extraction."""

    def test_jobs_flag_accepted(self, cli_runner, tmp_path):
        """-j flag is accepted without error."""
        py_file = tmp_path / "example.py"
        py_file.write_text("def func():\n    pass\n")

        result = cli_runner.invoke(main, ["scan", "-j", "2", str(tmp_path)])
        assert result.exit_code == 0

    def test_jobs_with_number(self, cli_runner, tmp_path):
        """-j N accepts a worker count."""
        py_file = tmp_path / "example.py"
        py_file.write_text("def func():\n    pass\n")

        result = cli_runner.invoke(main, ["scan", "-j", "4", str(tmp_path)])
        assert result.exit_code == 0

    def test_jobs_long_form(self, cli_runner, tmp_path):
        """--jobs flag works."""
        py_file = tmp_path / "example.py"
        py_file.write_text("def func():\n    pass\n")

        result = cli_runner.invoke(main, ["scan", "--jobs", "2", str(tmp_path)])
        assert result.exit_code == 0
